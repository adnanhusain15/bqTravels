      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-7 ">
        ...
        {appConstants.services.map((item) => {
          return (
            <div
              key={item.title}
              className="flex flex-col justify-end items-start aspect-video md:aspect-square p-3 rounded-sm"
              style={{
                backgroundSize: "cover",
                backgroundImage: `linear-gradient(0deg, rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 0.3)),url(${item.image})`,
              }}
            >
              <p className="font:lg sm:font-xl text-white font-medium">
                {item.title}
              </p>
              <p className="font:xl sm:text-2xl text-white  mb-3 font-semibold">
